...
| Signer WYSIWYG portal experience | Partial | `T318` shipped the inline PDF overlay surface; `T319` shipped guided navigation; `T320` shipped portal signature/stamp capture; `T321`-`T322` remain required before `T40` | PDF-first signing, guided next-field/keyboard flow, and portal signature/stamp capture are now in place, but parity integration and final closeout coverage are still open |
...
- Runtime token expiry policy is `min(72 hours from issuance, request.expires_at)`; `expired_token` is now active on signer JSONRPC routes only, while public GET routes keep the existing redirect-only denial behavior.
...
- Token lifecycle metadata now lives on `open.sign.request.signer` (`email_token_issued_at`, `email_token_expires_at`, `email_token_revoked_at`) rather than in a separate token model.
- Email-token audit taxonomy for this track is now `token_issued`, `token_opened`, `token_rejected`, and `token_revoked`, with token-safe metadata and schema-v1 audit-row export helpers (`T314`).
...
- [x] `T317a` Add live-HTTP overlap verification for portal signer flows using an out-of-band harness that bypasses `HttpCase`/`TestCursor` request serialization.
- [x] `T310` Define and document the external email-signer token strategy: reuse signer `portal.mixin` `access_token`, keep the existing signer route family, enforce token-only external access, accept baseline link-sharing risk, keep links multi-use until rotated/revoked/expired, and defer `expired_token` activation to `T313`.
...
- [x] `T312` Implement public token entry and email-only signer context resolution on `signer_id + access_token`, with no email-based ACL authorization path.
...
- [x] `T319` Add guided field navigation and keyboard flow to the WYSIWYG signer surface, including active-field state, `Next field`, `Tab` / `Shift+Tab`, cross-page focus/scroll, completed-vs-incomplete cues, and first-invalid focus on save/submit validation failure. The portal page payload now carries the minimal snapshot-backed validation metadata needed for ordered navigation (`sequence`, `min_length`, `max_length`, `validation_regex`), the inline PDF surface now owns active/complete/incomplete/invalid UI state plus summary mirroring, `Next field` lives in the PDF-surface header, and save/submit now reuse shared `open_sign_web` client validation before RPC without changing the public JSONRPC envelope. Post-closeout hardening is also complete: `Next field` now walks relative to the current active field in full actionable order even when that field is already complete, and guided-navigation affordances now disable themselves when the inline PDF surface is unavailable or fails to load.
- [x] `T320` Implement portal signature/stamp capture directly on the overlay surface, remove the current unsupported-field blocker for required signature/stamp fields, reuse the `T25` adoption/capture strategy, and preserve the existing attachment-backed payload contract and server-authoritative validation/storage rules. Post-closeout hardening is also complete: portal-created capture attachments are now signer-bound and provenance-verified on save/submit/page serialization/preview, malformed or forged attachment ids fail through the existing validation path, the preview route returns `404` when stored attachment provenance or metadata no longer match the signer/field contract, and the capture-create JSONRPC route now returns explicit structured error codes instead of parsing translated exception text.
...
| `2026-03-17` | Codex | Completed `T312` by making external token entry and internal partner fallback explicit in the portal controller and by locking denial-path coverage for the email-only signer threat model. External signer access is now explicitly resolved from `signer_id + access_token`, matching an internal user by email still does not grant access, and exact linked internal fallback remains limited to `partner_id` while still working even if a wrong/stale token is present. The public page/document/JSONRPC denial contract is unchanged (`/my` redirect for GET, `invalid_token` for JSONRPC). To preserve the separate internal preview surface while tightening no-token document access, the preview page now requests its PDF through an internal-only `preview=1` document link rather than relying on generic no-token document fallback. Revalidated with `/open_sign_portal`, `/open_sign`, and `scripts/review_gate_open_sign.sh --skip-web` (passed). |
| `2026-03-17` | Codex | Completed `T313` by turning signer token lifecycle metadata into enforced runtime behavior. Current signer-token access now classifies exact current-token matches as `valid`, `expired`, `revoked`, or `invalid` from lifecycle metadata on `open.sign.request.signer`; signer JSONRPC routes now emit `expired_token` for genuinely expired current links while public GET routes keep redirect-only denial; rotated/revoked/tampered/missing/metadata-incomplete links still map to `invalid_token`; a new system-only throttle layer in `open.sign.portal.token.throttle`, `open.sign.portal.token.throttle.attempt`, and `token_security_service` now provides rolling-window signer+IP invalid-attempt tracking with concurrency-safe invalid-attempt persistence, best-effort non-blocking clear on valid token-auth success, and no new public throttle error code; readonly `/document` now participates in the same record-and-clear throttle lifecycle; and exact linked internal fallback still works with wrong/revoked/expired token input while no longer echoing stale token state back into page DOM, document URLs, refresh URLs, or success redirects. Revalidated with targeted T313 suites, full `/open_sign_portal`, full `/open_sign`, and `scripts/review_gate_open_sign.sh --skip-web`. |
| `2026-03-18` | Codex | Completed `T314` by extending the audit/evidence taxonomy for the external email-signer token lifecycle. `open.sign.audit.log` now supports `token_issued`, `token_opened`, `token_rejected`, and `token_revoked`; signer token issuance and explicit manager revoke now append token-safe lifecycle chronology from `open_sign_portal`; successful token-authenticated page/document entry now emits `token_opened` once per distributed token; current-token lifecycle denials for `expired` and explicit hidden-token `revoked` states now emit deduplicated `token_rejected`; and `open.sign.audit.log` plus `open.sign.request` now expose internal schema-v1-shaped audit export helpers without adding a public export endpoint. `EVIDENCE_SCHEMA_V1.md` was aligned to the full current runtime audit taxonomy, including notification/idempotency/contact-correction events and the new token events. Revalidated with targeted audit/token-audit suites, full `/open_sign_portal`, full `/open_sign`, and `scripts/review_gate_open_sign.sh --skip-web`. |
...
| `2026-03-26` | Codex | Completed `T319` by turning the T318 inline signer surface into a guided signing flow without changing the public portal contract. `open_sign_portal` now exposes minimal snapshot-backed ordering/validation metadata (`sequence`, `min_length`, `max_length`, `validation_regex`) on portal field payloads, the inline PDF surface now owns active/complete/incomplete/invalid state plus clickable summary mirroring, `Next field` and `Tab` / `Shift+Tab` now move across actionable overlay fields in snapshot order with cross-page scroll/focus, and save/submit now reuse shared `open_sign_web` client validation to block invalid field payloads and focus the first invalid field before RPC while leaving server-side validation authoritative. Revalidated with targeted portal scaffold and HOOT suites, full `/open_sign_portal` (`367 tests, 0 failed`), full `/open_sign` (`111 tests, 0 failed`), and `scripts/review_gate_open_sign.sh` on fresh database `test_open_sign_t319`. |
| `2026-03-26` | Codex | Closed the `T319` hardening follow-up without changing the public portal contract. `Next field` now walks relative to the current active field in the full actionable ordering cursor instead of searching only inside the incomplete-field subset, so completed active fields no longer cause backward jumps to the earliest incomplete field. Guided-navigation affordances now also disable themselves when the inline PDF surface is unavailable or fails to load: the page suppresses the `Next field` button when no `pdf_render_url` is available, and the client disables `Next field` plus summary jump controls when the viewer never becomes interactive. Revalidated with targeted portal HOOT, targeted `test_portal_scaffold.py` (`103 tests, 0 failed`), full `/open_sign_portal` (`368 tests, 0 failed`), full `/open_sign` (`111 tests, 0 failed`), and `scripts/review_gate_open_sign.sh` on `test_open_sign_t319_followup`. |
| `2026-03-27` | Codex | Completed `T320` by turning portal signature/stamp overlays into real capture fields on the inline signer surface and then closing the immediate hardening follow-up before task closeout. `open_sign_portal` now reuses the shared adoption dialog on `/my/sign/<signer_id>`, persists sanitized signature/stamp metadata plus `signed_payload_attachment_id`, streams persisted previews through a signer-scoped payload route, and treats signature/stamp fields as actionable steps in guided navigation and payload collection. Hardening closed the trust boundary that review found: portal-created capture attachments are now stored as signer-bound `ir.attachment` rows with compact JSON provenance, save/submit/page serialization/preview all revalidate that provenance plus actual mimetype/size, forged or stale attachment ids fail through the existing validation envelope, and the create route now emits explicit structured error codes (`invalid_capture_field`, `readonly_session`, `request_locked`, etc.) instead of inferring them from translated exception text. Revalidated with targeted `test_portal_scaffold.py` (`114 tests, 0 failed`), shared `/open_sign_web` JS regression (`48 tests, 0 failed`), full `/open_sign_portal` (`379 tests, 0 failed`), full `/open_sign` (`111 tests, 0 failed`), and `scripts/review_gate_open_sign.sh --skip-web` on `test_open_sign_t320_capture`. |
