...
| Signer WYSIWYG portal experience | Partial | `T318` shipped the inline PDF overlay surface; `T319` shipped guided navigation; `T320`-`T322` remain required before `T40` | PDF-first signing plus guided next-field/keyboard flow are now in place, but portal signature/stamp capture and final parity coverage are still open |
